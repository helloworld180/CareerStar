  ...
  <view class="follow-page">
    ...
    <view class="header">
    	...
    	<view class="search-bar">
    	  ...
    	  <view class="search-input">
    		<input type="text" placeholder="搜索我的关注" placeholder-class="placeholder"/>
    	  </view>
    	</view>
    </view>
	...
	<view class="remain">
		...
		<view class="follow-header">
			<view class="follow-count">我的关注 ({{ followCount }})</view>
			<view class="sort-btn" @tap="showSortOptions">
				{{ currentSort }}
				<image src="../../static/applicant-mine/attention/sort.png" style="width: 34rpx; height: 34rpx; margin-left: 10rpx;"></image>
			</view>
		</view>
		...
		<scroll-view scroll-y class="follow-list" @scrolltolower="loadMore" @refresherrefresh="onRefresh":refresher-enabled="true":refresher-triggered="isRefreshing">
			<view v-for="(item, index) in followList" :key="item.id" class="follow-item":class="{ 'top': item.isTop }">
				...
				<view class="action-btn" :class="{ 'mutual': item.isMutual }">
					{{ item.isMutual ? '互相关注' : '已关注' }}
				</view>
				<view class="more-btn" @tap="showMoreOptions(item)">
					<image src="../../static/applicant-mine/attention/moreBtn.png" style="width: 48rpx; height: 48rpx;"></image>
				</view>
			</view>
		</scroll-view>
	</view>
    ...
    <uni-popup ref="morePopup" type="bottom">
		<view class="more-options">
			<view class="option" @tap="toggleTopStatus">
				<text>在关注列表置顶</text>
				<switch :checked="currentUser.isTop" @change="toggleTopStatus" />
			</view>
			<view class="option" @tap="showSetRemarkPopup">
				<text>设置备注名</text>
				<image src="../../static/applicant-mine/attention/next.png" style="width: 48rpx; height: 48rpx;"></image>
			</view>
			<view class="option cancel" @tap="showUnfollowConfirm">取消关注</view>
		</view>
    </uni-popup>

    <!-- 设置备注名弹窗 -->
    <uni-popup ref="remarkPopup" type="center" style="display: flex; flex-direction: column;">
      <view class="remark-popup">
        <view class="title">修改备注</view>
		<view class="subtitle">最多不超过12个字</view>
        <input
          type="text"
          v-model="newRemark"
          maxlength="12"
		...
		<view class="btn-group">
          <button @tap="cancelSetRemark">取消</button>
          <button @tap="confirmSetRemark" class="confirm">确认</button>
        </view>
      </view>
    </uni-popup>
...
export default {
  ...
  methods: {
    ...
    async toggleTopStatus() {
      try {
        await uni.request({
          url: `your_api_url/toggle-top/${this.currentUser.id}`,
          method: 'POST'
        })
        this.currentUser.isTop = !this.currentUser.isTop
        await this.fetchFollowList(true)
    ...
    showSetRemarkPopup() {
      this.$refs.morePopup.close()
      this.newRemark = this.currentUser.remark || ''
      this.$refs.remarkPopup.open()
    },
    async confirmSetRemark() {
      try {
        await uni.request({
          url: `your_api_url/set-remark/${this.currentUser.id}`,
          method: 'POST',
          data: { remark: this.newRemark }
        })
        this.currentUser.remark = this.newRemark
...
.btn-group .confirm, .unfollow-btn-group .confirm {
  color: #004934;
